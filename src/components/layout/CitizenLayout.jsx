    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', background: '#F8FAFC', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <header style={{ background: '#fff', borderBottom: '1px solid #F1F5F9', padding: '0 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 56, flexShrink: 0, boxShadow: '0 1px 3px rgba(0,0,0,0.04)', position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button onClick={handleLogout} style={{ width: 32, height: 32, borderRadius: 10, border: '1px solid #FEE2E2', background: '#FFF5F5', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.2s' }}
      ...
      {/* BOTTOM NAV */}
      <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#fff', borderTop: '1px solid #F1F5F9', boxShadow: '0 -4px 20px rgba(0,0,0,0.06)', zIndex: 50, paddingBottom: 'env(safe-area-inset-bottom)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-around', padding: '8px 4px' }}>
          {navItems.map(({ to, icon: Icon, label, end }) => (
            <NavLink key={to} to={to} end={end} className="nav-link-item"
              style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, padding: '4px 12px', borderRadius: 12, minWidth: 48 }}>
              {({ isActive }) => (
                <>
