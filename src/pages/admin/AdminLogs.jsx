    ...
    <div className="p-6 space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Registro de actividad y eventos</p>
        </div>
        <button onClick={load} className="flex items-center gap-2 px-4 py-2 border border-gray-200 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-50">
      ...
      <div className="card">
        {loading ? (
          ...
          <div className="divide-y divide-gray-50">
            {logs.map((log, i) => (
              <div key={log.id ?? i} className="px-6 py-4 flex items-start gap-4 hover:bg-gray-50 transition-colors">
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium mt-0.5 whitespace-nowrap ${levelColors[log.level] ?? 'bg-gray-100 text-gray-600'}`}>
                  {log.level ?? 'INFO'}
                </span>
