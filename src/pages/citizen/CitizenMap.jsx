...
const createIcon = (color, isMine, isCritical = false) => L.divIcon({
  className: '',
  html: `
    <div class="marker-pin-wrap ${isCritical ? 'marker-pin-critical' : ''}" style="width:${isMine ? 32 : 26}px;height:${isMine ? 32 : 26}px;">
        ...
        <MapContainer center={CENTER} zoom={13} style={{ height: '100%', width: '100%' }} scrollWheelZoom>
          ...
          {displayed.map((inc, index) => (
            <Marker
              ...
              icon={createIcon(priorityColor[inc.priority] ?? '#6b7280', mineIds.has(inc.id), inc.priority === 'CRITICAL')}
              eventHandlers={{
                add: (e) => {
                  const el = e.target?._icon
                  if (el) {
                    el.style.animationDelay = `${Math.min(index * 80, 600)}ms`
                  }
                },
              }}
            >
